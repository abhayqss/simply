...
export default function clientDocumentCountReducer (state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case CLEAR_CLIENT_DOCUMENT_COUNT:
            return state.removeIn(['error'])
                .setIn(['isFetching'], false)
                .setIn(['shouldReload'], action.payload || false)
                .removeIn(['value'])

        case CLEAR_CLIENT_DOCUMENT_COUNT_ERROR:
            return state.removeIn(['error'])

        case LOAD_CLIENT_DOCUMENT_COUNT_REQUEST: {
            return state.setIn(['error'], null)
                .setIn(['shouldReload'], false)
        }

        case LOAD_CLIENT_DOCUMENT_COUNT_SUCCESS:
            return state.removeIn(['error'])
                .setIn(['value'], action.payload)

        case LOAD_CLIENT_DOCUMENT_COUNT_FAILURE:
