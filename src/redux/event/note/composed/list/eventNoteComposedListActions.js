...
export function clear () {
    return { type: CLEAR_EVENT_NOTE_COMPOSED_LIST }
}

export function clearError () {
    return { type: CLEAR_EVENT_NOTE_COMPOSED_LIST_ERROR }
}

export function clearFilter () {
    return { type: CLEAR_EVENT_NOTE_COMPOSED_LIST_FILTER }
}

export function changeFilter (changes, shouldReload) {
    return {
        type: CHANGE_EVENT_NOTE_COMPOSED_LIST_FILTER,
...
export function load (config) {
    ...
    return dispatch => {
        dispatch({ type: LOAD_EVENT_NOTE_COMPOSED_LIST_REQUEST, payload: config.page })
        return service.findComposed(config).then(response => {
            const { page, size } = config
            const { data, totalCount } = response
            dispatch({
                type: LOAD_EVENT_NOTE_COMPOSED_LIST_SUCCESS,
                payload: { data, page, size, totalCount }
            })
        }).catch(e => {
            dispatch({ type: LOAD_EVENT_NOTE_COMPOSED_LIST_FAILURE, payload: e })
        })
    }
}
